export const term = {
  ...
  content: {
    ...
    methodology: {
      id: "Trigger didaftarkan pada kombinasi spesifik: tabel target, jenis event (INSERT/UPDATE/DELETE), dan **timing** (BEFORE/AFTER). Ketika event yang sesuai terjadi pada tabel tersebut, database secara otomatis memanggil fungsi trigger yang sudah didefinisikan sebelum operasi tersebut dianggap selesai. Di dalam fungsi trigger, kamu punya akses ke representasi baris lama (`OLD`) dan/atau baris baru (`NEW`) tergantung jenis event-nya, dan bisa memodifikasi nilai `NEW` sebelum benar-benar disimpan (khusus BEFORE trigger), atau melakukan aksi tambahan seperti menulis ke tabel lain (umum untuk AFTER trigger, seperti audit log). Setelah trigger selesai dieksekusi, operasi asli **dilanjutkan seperti biasa**.",
      en: "",
    },
    objective: {
